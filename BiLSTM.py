...
def BiLSTM_Model(city):
    ...
    # design network
    model = Sequential()
    model.add(Bidirectional(LSTM(4), input_shape=(train_X.shape[1], train_X.shape[2])))
    model.add(Dense(1))
    ...
    print("Train Score: ", trainScore.round(2))
    print("Test Score: ", testScore.round(2))
    ...
    pyplot.title(city)
    pyplot.legend()
    pyplot.savefig('./BiLSTM/' + city + ' Bidirectional LSTM.png', dpi=300, bbox_inches='tight')
    pyplot.show()